<template>
  <div>
    ...
    >
      ...
      <a-form-item label="容器端口" name="ports2" :rules="[{ required: true, message: '输入一个容器端口!' }]">
        <a-input placeholder="请输入容器端口" size="large" v-model:value="formState.ports2" />
      </a-form-item>
...
const formState: any = reactive<FormState>({
  containerName: 'my-httpd-container',
  ports1: 80,
  ports2: 80,
  remember: true
})
...
const verifyForm = async (callback: () => void) => {
  await formRefs.value.validate().then(async () => {
    ...
    const command = `docker run --name  -p 80:80  -d httpd:latest`
