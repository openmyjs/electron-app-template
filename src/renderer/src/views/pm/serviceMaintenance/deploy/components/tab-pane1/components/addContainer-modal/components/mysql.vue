...
const verifyForm = (callback: (state: boolean) => void) => {
  formRefs.value.validate().then(async () => {
    emit('confirmLoading', true)
    const command = `docker run --name ${formState.containerName} -e MYSQL_ROOT_PASSWORD=${formState.account} -e MONGO_INITDB_ROOT_PASSWORD=${formState.password} -p ${formState.ports1}:${formState.ports2} -d ${formState.remember ? ' --restart=always' : ''} mysql:${props.tag} --auth`
